...
int load(char* filename, int segment)
{
    ...
    printf("gloader loading %s to segment %x ... ", filename, segment);
    ...
    {
        w[j] = get_word(segment+2, i); // get word gets 2 bytes at a time
        //put_word(w[j], segment, i);    // therefore set 2 bytes
        j++;
    }
    j=0;
    for (i =0; i<=(tsize + dsize); i+=2)
    {
        //w[i] = get_word(segment+2, i); // get word gets 2 bytes at a time
        put_word(w[j], segment, i);    // therefore set 2 bytes
        j++;
    }*/
